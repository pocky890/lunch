...
- 卡牌使用流程：doJoin 先以 transaction 原子認領（從 `userCards` 移除，防與盜牌卡競態），寫入 `pendingCardReturns/{date}`（含 `name`），開獎時 `reconcilePendingCardReturns` 統一結算（含補掃過去日期的滯留記錄）。商店購買的卡帶 `fromShop: true`，任何歸還路徑（reconcile / returnCardToPool / 離職回收）都不回 `cardPool`（商店庫存與卡牌池獨立）。
- 用卡後取消參加的退還規則：`IMMEDIATE_EFFECT_CARD_TYPES`（reveal_numbers、steal_card）使用當下就已生效，取消一律不退還、直接進 `cardPool`。其餘卡種（follow_restaurant/extra_number/thirsty_card/nuke_card/birthday_card）效果要等開獎才生效，`reconcilePendingCardReturns` 會檢查 `weekly/{date}.participantNames` 是否包含該人——當天最終沒有留在參加名單裡（效果從未套用）就直接退還卡牌到本人 `userCards`（不進公共池）；若又重新加入，就照正常流程進池子。
- 防重複用卡：`cardUsedToday/{date}/{key}` 是獨立於 `participants` 的持久標記，用卡當下就寫入、取消參加（`doCancel` 刪 `participants` 記錄）不會清掉，避免「用卡→取消→重買一張→再加入」無限重複用卡/偷卡的漏洞。`doReset`（管理員清除今日遊戲）會一併清掉這個標記。
- 卡牌是否可用取決於 `hasUsedCard`（`myEntry?.cardUsed` 存在，或 `cardUsedToday/{date}/{key}` 標記存在），不是單純看是否已加入（`myEntry`）。當天已加入但還沒用卡的人，回來編輯（`isEditing`）時仍可補選一張卡使用（號碼欄位鎖死，`NumberPicker` 用 `lockedKeys` 擋掉主號碼被更改，但加號卡/生日卡需要的額外號碼欄位仍可填寫）；已經用過卡的人（不管是編輯、還是取消參加後重新加入）手牌選擇 UI 都會直接隱藏，前端用即時監聽的 `usedCardToday` state（來源同 `cardUsedToday`）判斷，避免取消/編輯後看起來還能選卡、送出才被後端擋掉的落差。
